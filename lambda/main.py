...
import logging
from helpers import extract_file_name, read_file_contents_from_s3, write_to_s3, save_job_run_details, rename_columns,\
    get_mapping_tables
...
def process_file(bucket, key):
    try:
        ...
        file_name = extract_file_name(bucket)
        response = write_to_s3(cleaned_data, destination_bucket, destination_prefix, file_name)
        # time for dynamodb logging
        end_time = datetime.datetime.now()
        elapsed_time = end_time - start_time
        elapsed_seconds = elapsed_time.total_seconds()
        save_job_run_details(
            lambda_job_id, job_start_time, f"s3://{bucket}/{key}", response['destination_file_name'], start_time, end_time,
            elapsed_seconds, response['status_code'], dynamodb_table)
