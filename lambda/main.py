...
import logging
from helpers import extract_event_name, read_from_s3, write_to_s3, save_job_run_details, rename_columns,\
...
def process_file(bucket, key):
    try:
        ...
        res = write_to_s3(cleaned_data, destination_bucket, destination_prefix, event_name)
        # time for dynamodb logging
        end_time = datetime.datetime.now()
        elapsed_time = end_time - start_time
        elapsed_seconds = elapsed_time.total_seconds()
        save_job_run_details(
            lambda_job_id, job_start_time, f"s3://{bucket}/{key}", res['destination_file_name'], start_time, end_time,
            elapsed_seconds, res['status_code'], dynamodb_table)
